...
double swim(struct fish *fish) {
    ...
    #pragma omp parallel for schedule(static, CHUNK_SIZE) reduction(max:max) private(prev_x, prev_y, new)
    for (int i = 0; i < NUM_FISH; i++) {
        if (fish[i].w < 2 * INITIAL_WEIGHT) {
            prev_x = fish[i].x;
            prev_y = fish[i].y;
...
double find_barycentre(struct fish *fish) {
    ...
    #pragma omp parallel for schedule(static, CHUNK_SIZE) reduction(+:numerator) reduction(+:denominator) private(distance)
    for (int i = 0; i < NUM_FISH; i++) {
